 ...
 * @description Profil Belote avec vraies valeurs (stats/coins/achievements/
...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 2) /leaderboards/belote/my-rank : best-effort
      try {
        log.bin('GET /leaderboards/belote/my-rank');
        const r = await api.getMyRank('belote', 'season');
        ...
        const msg = e?.message ?? e?.toString?.() ?? 'unknown';
        const status = e?.status ?? '?';
        if (status === 404) log.explain('pas encore de rang belote pour ce joueur');
