 ...
 * @description Leaderboard Belote avec 3 portées : Mondial / Pays / Ville.
 * Données réelles depuis /leaderboards/belote?scope=<...>, auto-détection
...
export default function LeaderboardScreen() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      log.bin(`GET /leaderboards/belote`, { scope, filter });
      const r = await api.getLeaderboardScoped('belote', filter, scope, 50);
      log.bout('200 /leaderboards/belote', `${r.entries.length} entrées`);
