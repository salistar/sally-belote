/**
 * @file login.tsx
 * @description Login screen for the Belote app. Card image logo, email/password authentication, guest mode, and demo credentials.
...
const CARD_IMAGE = require('../../assets/cards/11C.png');
...
export default function LoginScreen() {
  ...
  useEffect(() => {
    console.log('[Belote/Login] Component mounted');
  }, []);

  const handleLogin = async () => {
    if (!email || !password) {
      console.log('[Belote/Login] Validation failed: empty fields');
      Alert.alert(t('error'), t('fillAllFields'));
      return;
    }
    setLoading(true);
    try {
      console.log('[Belote/Login] Fetching login...');
      await api.login(email, password, { gameType: 'belote' });
      console.log('[Belote/Login] Login successful');
      router.replace('/(tabs)');
    } catch (e: any) {
      console.error('[Belote/Login] Error:', e);
  ...
  const { promptAsync, ready: googleReady, missingClientId, missingNative } = useGoogleSignIn(
    async (idToken: string) => {
      setLoading(true);
      try {
        await api.loginWithGoogle(idToken, { gameType: 'belote' });
        console.log('[Belote/Login] Google login successful');
        router.replace('/(tabs)');
      } catch (e: any) {
        console.error('[Belote/Login] Google error:', e);
        Alert.alert(t('error'), e.message || 'Google sign-in failed');
      } finally {
        setLoading(false);
      }
    },
    (msg: string) => { console.warn('[Belote/Login] Google:', msg); setLoading(false); }
  );
  ...
  const handleGuest = async () => {
    setLoading(true);
    try {
      console.log('[Belote/Login] Fetching guest session...');
      await api.createGuestSession();
      console.log('[Belote/Login] Guest session created');
      router.replace('/(tabs)');
    } catch (e: any) {
      console.error('[Belote/Login] Error:', e);
...
/* === End of login.tsx -- Belote -- SallyCards === */
